...
\begin{multicols*}{4}
  ...
  \begin{enumerate}
    ...
    \item retrieve qualified data records via RID lookups
          \( {\displaystyle{
                N_{\text{lookup}}=
                \begin{cases}
                  0                                & \text{if $$I$$ is covering,} \\
                  \min\{||\sigma_{p_c}(R)||, |R|\} & \text{otherwise}
                \end{cases}
              }} \)
          \begin{itemize}
            \item $N_{\text{lookup}} \geq \lceil\frac{||\sigma_{p_c}(R)||}{b_d}\rceil$
          \end{itemize}
  \end{enumerate}
  ...
  \begin{enumerate}
    \item $N_\text{dir} = 1$ if the index is an extendible hash index, 0 otherwise.
    \item $N_\text{bucket}$ = number of index's primary/overflow pages accessed.
          \begin{itemize}
            \item $N_\text{bucket} \leq M_\text{bucket}$, the maximum number of primary/overflow pages for a bucket.
          \end{itemize}
    \item $N_\text{lookup}$ = number of pages accessed to retrieve the matching data records.
          \begin{itemize}
            \item \( {\displaystyle{
                        N_{\text{lookup}}=
                        \begin{cases}
  ...
  \begin{center}
    $|R| + 2\vert \pi^*_L(R) \vert (\lceil \log_{B-1}(N_0) \rceil + 2)$
  \end{center}

  \begin{enumerate}
    \item initial sorted runs: $N_0 = \lceil \vert \pi^*_L(R) \vert / B \rceil$
    \item extract attributes: $|R|$ scan + $\vert \pi^*_L(R)\vert$ output temp result
    \item sort records: $2\vert \pi^*_L(R) \vert (\log_{B-1}(N_0) +1)$
    \item remove duplicates: $\vert \pi^*_L(R)\vert$ to scan records
  \end{enumerate}

  \subsection{Optimized sort-based approach}
  ...
  \begin{center}
    $|R| + 2\vert \pi^*_L(R) \vert \lceil \log_{B-1}(N_0^{\text{opt}}) \rceil$
  \end{center}

  \begin{enumerate}
    \item initial sorted runs: $N_0^{\text{opt}} = \lceil \vert \pi^*_L(R) \vert / (B-1)\rceil$
    \item cost to create initial sorted runs: $|R| + \vert \pi^*_L(R) \vert$
    \item cost of merging passes excluding cost to output $\pi^*_L(R)$: $(2 \lceil \log_{B-1}(N_0^{\text{opt}}) \rceil - 1) \vert \pi^*_L(R) \vert$
  \end{enumerate}
  ...
  \subsection{Sort-based vs Hash-based}

  \begin{itemize}
    \item if $B>\sqrt{|\pi^*_L(R)|}$, same I/O cost as hash-based approach
          \begin{itemize}
            \item $N_0 = \lceil \frac{|\pi^*_L(R)|}{B-1} \rceil \approx \sqrt{|\pi^*_L(R)|}$ initial sorted runs
